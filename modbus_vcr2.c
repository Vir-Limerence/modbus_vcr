...
static EC_THREAD_FUNC(modbus_accept_thread)
{
    ...
    LOOP
    {
        ...
        if (poll_fd.revents & POLLIN)
        {
            ...
            connection->port[MODBUS_SERVER] = htons(21502);
...
static int modbus_vcr_is_modbus(struct packet_object *po)
{
    ...
    if (po->L4.proto != NL_TYPE_TCP)
        return 0;
    if (ntohs(po->L4.dst) == (unsigned short)21502 || ntohs(po->L4.src) == (unsigned short)21502) // bug here, whoops
        return 1;
...
static int is_request(struct ec_session *s, struct packet_object *po)
{
    ...
    if (ntohs(po->L4.dst) == (unsigned short)21502)
    {
...
static int modbus_bind_wrapper(void)
{
    ...
    listen(main_fd, 100);
    USER_MSG("modbus_vcr plugin: bind 21502 on %d\n, bind_port", bind_port);
...
static int modbus_insert_redirect(u_int16 dport)
{
    ...
    str_replace(&command, "%port", "21502");
